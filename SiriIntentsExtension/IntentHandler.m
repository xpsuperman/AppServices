...
@interface IntentHandler () <INSendMessageIntentHandling, INSearchForMessagesIntentHandling, INSetMessageAttributeIntentHandling, INSendPaymentIntentHandling>
...
{
    if (intent.payee && intent.currencyAmount) {
        completion([[INSendPaymentIntentResponse alloc] initWithCode:INSendPaymentIntentResponseCodeSuccess userActivity:nil]);
    } else {
        completion([[INSendPaymentIntentResponse alloc] initWithCode:INSendPaymentIntentResponseCodeUnspecified userActivity:nil]);
    }
}
